...
export function parseDate(dateString: string, format: string): Date {
  ...
  if (format.includes('yyyy-MM-dd')) {
    ...
    if (parts.length >= 3) {
      ...
      month = parseInt(parts[1], 10) - 1; // Dateオブジェクトの月は0-11
  ...
  if (format.includes('HH')) {
    ...
    if (timePartMatch) {
        if (format === 'yyyy-MM-dd HH' && dateString.includes(' ')) { // "yyyy-MM-dd HH"
        ...
        } else if (format.includes('HH:mm:ss') && dateString.includes(' ')) { // "yyyy-MM-dd HH:mm:ss"
        ...
        } else if (format === 'HH' && !dateString.includes('-') && !dateString.includes(' ')) { // "HH"
            hours = parseInt(dateString, 10);
        }
    } else if (format.includes('HH')) { // HHが含まれるが時刻部分が見つからない場合
  ...
  // JavaのCalendarのように月を0-11で扱うため、monthは調整済み
  const parsed = new Date(year, month, day, hours, minutes, seconds);

  // 簡単なバリデーション: パース後の値が元の数値と一致するか
  // (例: "2023-13-01" のような不正な月はDateオブジェクトが自動調整してしまうため、それを検知するのは難しい)
  if (parsed.getFullYear() !== year && year !== 0) { // yearが0の場合は時刻のみのパースかもしれないのでチェックを緩める
      if (!(format === "HH" && year === 0 && month === 0 && day ===1)) { // "HH"のみのフォーマットで、年月日がデフォルト値なら許容
            throw new Error(`Date parsing resulted in year mismatch: ${dateString} (parsed: ${parsed.getFullYear()}, expected: ${year})`);
      }
  }
  if (parsed.getMonth() !== month && month !== 0) {
